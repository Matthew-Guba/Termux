...
async def start_broadcast(update, context):
    ...
    return {
        'text': (
            ...
            "✨ Эмодзи автоматически поддерживаются!"
        ),
        'reply_markup': reply_markup
    }
...
async def handle_message(update, context):
    ...
    if setup['step'] == 'edit_text':
        ...
        if broadcast_id in scheduled_broadcasts:
            ...
            await update.message.reply_text(
                f"✅ Текст рассылки *{broadcast_id}* изменён!\n\n_{new_text[:100]}..._",
                parse_mode='Markdown',
                reply_markup=reply_markup
            )
            return "processing"
    ...
    if setup['step'] == 'edit_chats':
        ...
        if broadcast_id in scheduled_broadcasts:
            ...
            await update.message.reply_text(
                f"✅ Чаты рассылки *{broadcast_id}* изменены!\n\nЧатов: {len(processed_ids)}",
                parse_mode='Markdown',
                reply_markup=reply_markup
            )
            return "processing"
    ...
    if setup['step'] == 'edit_interval':
        ...
        try:
            ...
            if broadcast_id in scheduled_broadcasts:
                ...
                await update.message.reply_text(
                    f"✅ Интервал рассылки *{broadcast_id}* изменён на {new_interval} мин",
                    parse_mode='Markdown',
                    reply_markup=reply_markup
                )
                return "processing"
    ...
    if setup['step'] == 'chats':
        ...
        keyboard = [
            [InlineKeyboardButton("1️⃣ 1 мин", callback_data="interval_1")],
            [InlineKeyboardButton("5️⃣ 5 мин", callback_data="interval_5")],
            [InlineKeyboardButton("🕐 30 мин", callback_data="interval_30")],
            [InlineKeyboardButton("⏰ 1 час", callback_data="interval_60")],
            [InlineKeyboardButton("📅 1 день", callback_data="interval_1440")],
            [InlineKeyboardButton("❌ Отмена", callback_data="cancel_setup")]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        ...
        await update.message.reply_text(
            ...
            "Выбери кнопку или отправь своё число (минуты):",
            parse_mode='Markdown',
            reply_markup=reply_markup
        )
        return "processing"
    
    # Шаг 4: Получить интервал (если ввели текстом)
    if setup['step'] == 'interval':
        try:
            ...
            result = await finalize_broadcast(update, context, setup, interval)
    ...
    while True:
        ...
        try:
            ...
            for chat_id in broadcast['chats']:
                ...
                try:
                    # Использовать markdown=True для поддержки эмодзи
                    if broadcast['photo']:
...
async def list_broadcasts(update, context):
    ...
    for bid, broadcast in scheduled_broadcasts.items():
        ...
        keyboard.append([
            InlineKeyboardButton(f"📋 {bid}", callback_data=f"show_{bid}"),
            InlineKeyboardButton("⏸" if broadcast['active'] else "▶️", callback_data=f"toggle_{bid}"),
            InlineKeyboardButton("🗑", callback_data=f"confirm_delete_{bid}")
        ])
...
async def resume_broadcast(update, context):
    ...
    try:
        ...
        if broadcast_id in scheduled_broadcasts:
            ...
            return f"❌ Рассы��ка {broadcast_id} не найдена"
    except ValueError:
        return "❌ Неверный ID"
...
async def show_broadcast(update, context):
    ...
    try:
        ...
        if broadcast_id in scheduled_broadcasts:
            ...
            keyboard.append([InlineKeyboardButton("🗑 Удалить", callback_data=f"confirm_delete_{broadcast_id}")])
...
async def handle_callback(update, context):
    ...
    # Действия с ID рассылки
        ...
        keyboard = [
            [InlineKeyboardButton("✅ Да, удалить", callback_data=f"do_delete_{broadcast_id}")],
            [InlineKeyboardButton("❌ Отмена", callback_data=f"show_{broadcast_id}")]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        
        result = {
            'text': f"⚠️ Удалить рассылку *{broadcast_id}*?\n\nЭто действие нельзя отменить!",
            'reply_markup': reply_markup
        }
    ...
    # Редактирование
        ...
        keyboard = [[InlineKeyboardButton("❌ Отмена", callback_data=f"show_{broadcast_id}")]]
        reply_markup = InlineKeyboardMarkup(keyboard)
        
        result = {
            'text': f"✏️ Отправь новый текст для рассылки *{broadcast_id}*:",
            'reply_markup': reply_markup
        }
        ...
        keyboard = [
            [InlineKeyboardButton("🗑 Удалить фото", callback_data=f"remove_photo_{broadcast_id}")],
            [InlineKeyboardButton("❌ Отмена", callback_data=f"show_{broadcast_id}")]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    elif data.startswith("edit_chats_"):
        broadcast_id = int(data.split("_")[2])
        ...
        result = {
            'text': f"💬 Отправь новый список чатов через запятую для рассылки *{broadcast_id}*:",
            'reply_markup': reply_markup
        }
        ...
        keyboard = [
            [InlineKeyboardButton("1 мин", callback_data=f"set_interval_{broadcast_id}_1")],
            [InlineKeyboardButton("5 мин", callback_data=f"set_interval_{broadcast_id}_5")],
            [InlineKeyboardButton("30 мин", callback_data=f"set_interval_{broadcast_id}_30")],
            [InlineKeyboardButton("1 час", callback_data=f"set_interval_{broadcast_id}_60")],
            [InlineKeyboardButton("❌ Отмена", callback_data=f"show_{broadcast_id}")]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        
        result = {
            'text': f"⏰ Выбери новый интервал для рассылки *{broadcast_id}* или отправь своё число (минуты):",
            'reply_markup': reply_markup
        }
    ...
    # Быстрый выбор интервала при создании
